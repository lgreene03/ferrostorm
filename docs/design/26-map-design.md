...
mechanical: every feature must be placed as its full ORBIT under the map's
symmetry group. The shared library `tools/mapgen.py` writes each feature
together with every image of it, then proves, cell by cell, that blocked cells,
ferrite and bridges agree across each cell's whole orbit; that every apron is
open so the starting Construction Yard and MCV fit; that every start can reach
every ferrite patch, every other start and every apron over the passable cells
with bridges open; that closing every crossing leaves no start able to reach any
other, so the crossings are load-bearing rather than decorative; that the
ferrite distance profile is identical from every start; that the outposts are a
whole number of orbits and their distance profiles match from every start too;
and that terrain density sits inside 8 to 10 percent, below which the map reads
as empty and above which pathing and the draw-call budget suffer. A bad edit
fails in the generator, not in a match.

**The groups, and why there are only two** (P7-8b). The invariant used to be
stated as "a 180-degree rotation-symmetric pair", which is the `rot180` group:
the identity and the half turn, an orbit of two. That is still the default and
every map from skirmish-01 to skirmish-08 is built on it. `mirror2` adds the two
centre-line mirrors for an orbit of four, which is what a map with four starts
needs and what skirmish-09 uses. A group containing only the identity is the
asymmetric mission mode described below, so the three modes are one code path
rather than a boolean beside a special case. There is deliberately no 90-degree
rotation: a quarter turn carries a w-by-h rectangle onto an h-by-w one, so it is
a symmetry of SQUARE maps only, and not one of the maps in this pool is square.
Its absence is a decision and not an oversight, and a square map wanting an
orbit of four can use `mirror2`, which works on any rectangle.

**Start ORDER is part of the invariant, not a detail.** On a map whose group
contains the half turn, the starts must be ordered so that seats 0 and 1 are a
180-degree rotation pair, seats 2 and 3 the next one, and so on; mapgen asserts
it. The reason is practical rather than aesthetic. `rot180` is a member of every
group here, so a TWO-player game on a four-start map is fair in exactly the
sense every two-start map in this pool is fair, provided the two seats it fills
are a rotation pair. That is what makes it safe to offer a four-start map in the
theatre picker while the lobby still expresses two seats: the spare starts
simply go unused, and the two that are filled face each other across the map
centre.

The honest cost of a mirror group is handedness. Rotation is preferred to
reflection for 1v1 because it gives both players an identical experience of
every feature (section 2), and that still holds WITHIN each rotation pair. Two
seats related by a mirror rather than by the half turn see a handed copy of the
map instead of a repetition: one commander's back wall lies to its north-west
where the other's lies to its north-east. Measured fairness is untouched, and
mapgen proves the distance profiles match from all four starts, but the
experience across a mirror is a reflection. A four-seat game accepts that; a
two-seat game on the same map never meets it, because it is seated on a rotation
pair.
...
**skirmish-09, Kilnmoor Quarters (160x120), the pool's first FOUR-player map.**
19,200 cells, which is 4,800 per commander: between a duel map's 3,072 per seat
and the mid tier's 6,144, so the board is larger than anything below the big
theatre while a base still sits in duel-sized ground. It is a rectangle rather
than a square, which is why its symmetry is `mirror2` and could not have been a
quarter turn.

Every other map here is LINEAR, a river or a ridge or a sound lying between two
lands, and the game on it is which crossing you commit to. A four-seat map
cannot be linear, so this one is built around a centre. Two spoil dykes run the
full width and the full height of the moor and meet at the Kiln, a ruined
firing-house at the exact centre; they cut the moor into four quarters, one
commander to each. There are eight ways through: a pass part way along each dyke
arm, joining each quarter to its two neighbours, and four kiln gates where the
arms stop short of the Kiln. The four gates matter more than their number
suggests, because together they form a RING of open ground round the Kiln, which
is the only direct route between diagonal quarters and the shortest route
between any two. Being a ring, holding it means holding two gates rather than
one, which is what stops a single commander sitting on the centre. The mid-dyke
passes are the turning moves.

The dykes lie on the mirror axes, which is the one real constraint `mirror2`
puts on a shape: a feature sitting on an axis cannot wander across it, because a
centre line drifting left would be reflected into a second dyke drifting right.
So the dykes wander in WIDTH rather than in position, swelling and narrowing
along their length, which reads as banked spoil instead of a ruled wall and
costs the pathing nothing. The economy is 64 ferrite cells, 16 per seat, laid to
the section 2 split: a safe patch behind each base, a forward patch in each
quarter, and the richest ground of all against the Kiln wall where nobody can
mine it quietly. Eight outposts stand, two per commander.

Its orientation cost is stated rather than hidden: four quarters built as one
orbit look alike, and the decorative layer cannot tell them apart because
decoration is placed as an orbit too. The Kiln is the landmark a player
navigates by. Played by two, which is what the lobby allows today, it reads as a
long diagonal between seats 0 and 1 with a contested centre and two empty
quarters to manoeuvre through, and that is a different and good game.
