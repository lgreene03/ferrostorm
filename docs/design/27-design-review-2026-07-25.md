...
| DR-12 | The rifle-blob and static-defence convictions: promote the parked ADR-010 decision (rebalance defence or withdraw the expectation) and the per-cost review of the service rifle (wpn_service_rifle, renamed from TestRifle by DR-17) | M | MOVES | Balance + Game Designer co-sign (A11); the tool's rows stay visible every run meanwhile |
...
| DR-17 | DELIVERED. The three prototype weapon names are gone: wpn_test_cannon/rifle/rocket became wpn_tank_cannon / wpn_service_rifle / wpn_rocket_tube, and TestCannon/TestRifle/TestRocket became TankCannon/ServiceRifle/RocketTube. This row's NEUTRAL claim was VERIFIED rather than assumed: the ADR-006 catalogue checksum is 0x237BAC5BAFB3A254 before and after, because it hashes canonicalised defs and takes the resolved integer WeaponId, never the id string or the file bytes - so every existing save and replay still loads. Numbers untouched (those are DR-12's) | S | NEUTRAL, MEASURED (checksum and all 24 goldens identical) | nothing |
| DR-18 | FIFTH MAP DELIVERED: skirmish-05 "Ashford Reach", the river map at close quarters, carrying BOTH underused features (82 destroyable spans + 2 outposts). Inverts skirmish-04: the flanks are destroyable and the centre ford permanent, so felling a span closes a turning move and can never sever the theatre. Generated by tools/gen_skirmish_05.py per doc 26 (never hand-typed); proven walkable both ways by fordgate. SIXTH MAP ALSO DELIVERED: skirmish-06 "Sable Crossroads", which closes the other half of the complaint by changing what an outpost IS - four ridge-bound quadrants, the bases diagonally opposite, and all four outposts standing in the two NEUTRAL quadrants every attack route must cross, so an outpost is the objective rather than side income. mapgate's outpost assertion therefore proves traversal on this map (it captured 4 of 4), which it could not on the Ashford. ROW COMPLETE | S-M each | NEUTRAL (new files) | map-design doc 26 invariants apply; raised Q018 |
