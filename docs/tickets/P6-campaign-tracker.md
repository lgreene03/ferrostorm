...
| DR-05+06 | The selection layer: army key, type-select, group 0, idle-harvester key | doc 27 tier 2, first delivery wave off the design review | NEUTRAL (client only) | DONE - 102 harness checks |
...
| DR-14 | AI difficulty as economy, not wave size: the Easy-to-Brutal ladder (doc 27 tier 4) | doc 28 (design note) under GDD line 76; personality and difficulty split into orthogonal axes | NEUTRAL, MEASURED: Normal is the identity rung so all 24 goldens are byte-identical (doc 27 predicted MOVES and was wrong) | DONE in the sim - difficultygate (4 parts); the client picker is owed as a follow-up (doc 28 s7) |
| DR-14b | The ladder becomes REACHABLE: DIFFICULTY picker, sidecar + Hello plumbing, the handicap applied at world build | doc 28 s7 (the follow-up that wave owed); C4/C4b precedent | NEUTRAL (client only; the sim gained one read-only property) | DONE - 123 harness checks; the harness CAUGHT a bad check (a beat inferred in a world the AI could not afford to act in) |
| DR-18 | A fifth skirmish map, skirmish-05 "Ashford Reach", exercising the underused vocabulary | doc 27 tier 5; authored as a generator under doc 26's "never hand-typed" rule | NEUTRAL (new files; goldens byte-identical) | DONE - fordgate proves it walkable BOTH ways; the AI-offensive asymmetry it exposed is filed as Q018 rather than tuned away |
| DR-17 | The prototype weapon names leave the shipped catalogue (wpn_test_* and TestCannon/Rifle/Rocket) | doc 27 tier 5 | NEUTRAL, MEASURED: catalogue checksum 0x237BAC5BAFB3A254 unchanged (it hashes resolved def integers, never id strings), so existing saves and replays still load; 24 goldens byte-identical | DONE - the `vanguard_autocannon` missing-prefix inconsistency was found alongside and REPORTED rather than folded in, since it is outside this ticket |
| DR-18b | The sixth map, skirmish-06 "Sable Crossroads": outposts as the OBJECTIVE, not side income | doc 27 tier 5, completing the DR-18 row | NEUTRAL (new files; 24 goldens byte-identical) | DONE - 4 of 4 outposts AI-captured (the best in the pool, and on this map that result proves traversal); fordgate extended to prove the ring route threads both ways |
| DR-20 | The capture alert: GameEventType.Captured finally has a consumer | doc 27's "findings filed as defects" row, deferred there as a UX + audio call | NEUTRAL (client only; goldens byte-identical) | DONE - 128 harness checks; the fog case (a capture in the shroud stays SILENT) is checked as a pure decision because proving a negative end-to-end would test the fixture |
| MAP-CRIT-A | Design-review wave A: start-separation guard, decoration on skirmish-02/-04/-05, a second outpost pair on -02, doc 26's false skirmish-03 claim withdrawn | game-designer critique of the pool against doc 26 + the GDD | NEUTRAL (generators, maps and docs; 24 goldens byte-identical) | DONE - visible content roughly tripled on the three dressed maps; mapgate 14 outposts, 13 AI-captured |
| MAP-07 | skirmish-07 "Karsthollow Basin" (256x192): the epic theatre at the MEASURED ceiling, and the pool's first non-crossing map | design review findings 8 and 10 (epic theatre + a macro map), under the user's directive to go bigger | NEUTRAL (new files; 24 goldens byte-identical) | DONE - 8x a small map, 31.31% decorated (the most detailed in the pool), 128 ferrite cells in 22 patches (16 of them expansion sites), 6 outposts; basingate plays it to a RESULT in ~22 simulated minutes |
| MAP-08 | skirmish-08 "Tidewrack Sound" (128x96): the mid tier, and water as a BOUNDARY rather than a barrier | design review's size-distribution gap (nothing between 96x64 and 192x128) | NEUTRAL (new files; 24 goldens byte-identical) | DONE - no bridges and nothing to cross; basingate extended to cover BOTH chokeless maps and plays it to a result (186 destroyed, the most in the pool) |
