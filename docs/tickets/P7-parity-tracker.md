...
| P7-23 | GDD s3 line 25's PRECISION STRIKE, completing the Directorate's surgical pair - **and it could not be built**, because ADR-062 put ONE SupportPowerId on the def | GDD s3 line 25; GDD s8; ADR-044; ADR-062; ADR-063; Q021 | **The cardinality was wrong and the measurement proves it:** the Directorate owns THREE faction-exclusive buildings, of which the orbital cannon cannot carry a power (it already uses ChargeTicks for its own cycle) and the power plant is the first thing anyone raises - so one power per building caps the Directorate at ONE, and s8 asks for 3-4. Def now carries a LIST, and the powers on a building **SHARE its charge** - a design decision, so a Bastion holds a scan OR a strike ready and the choice is the player's. Rejected: independent charges per power (needs per-(entity,power) state, moves goldens, and buys nothing except never having to choose). Damage **300 = a THIRD of the orbital cannon's 900**, the same ratio ADR-062 gave the charge; radius = **the cannon's own 1.5-cell core, ONE BAND, no falloff** - a ring of half damage would make it a smaller superweapon, the one thing s8 says a minor power is not | **All 24 byte-identical, measured TWICE** (after the cardinality change and after the strike) - the charge stayed in Entity.ChargeTicks, which is exactly what the shared-charge decision bought; save unchanged at v13; **catalogue checksum moves to 0xA5544FFFAFAA8B5D** | **DONE** - ADR-064; `precisionstrikegate` (6 stages, CONTROL FIRST). Catches what nothing else could: orbitalscangate proves a power that REVEALS and asserts no damage, supportpowergate proves machinery with a power that does nothing, and **nothing anywhere asserted a building may hold more than one power**. Three bite tests, each landing on its own stage. It leaves FERRITE FIELDS ALONE, because s8 gives field destruction to the Sodality's seismic charge as its identity - proved to bite. Unblocks Q021's remaining three: the Sodality's five exclusive buildings can now carry them |
| P7-24 | GDD s3 line 30's RADAR JAMMING, the first of the Sodality's three dirty tricks - and the first support power whose effect lands on a PLAYER rather than on the map | GDD s3 line 30; GDD line 48; ADR-008; Q021 | **Measured first:** the radar has NO other mechanical effect in the sim, so jamming means exactly what ADR-008's blackout means and nothing more had to be invented - the client already carries the blanked minimap, toast, VO, cue and ping off one predicate. The jam lives in the SIM and the client ASKS it (`IsRadarJammed(LocalPlayerId)`), because a blackout the client decided for itself would differ between two LAN peers. Unlocked by the WATCH POST: sensor warfare in the sensor building, and it is unarmed/visible/killable BY DESIGN (GDD line 56) so s8's counterplay needs no arranging. Duration **166 ticks = a third of its own charge**, the ratio used twice already, giving a duty cycle a reader can check - blind at most a third of the time. Rejected: the scan's 75-tick reveal (tempting symmetry, too short to be a trick); the full charge (a permanent blackout, not a jam) | **All 24 byte-identical, measured**; **save bumps to v14** deliberately; **catalogue checksum moves to 0xD4F4248474EFA6E7** | **DONE** - ADR-065; `radarjamminggate` (5 stages, CONTROL FIRST). Catches what nothing else can: every earlier support-power gate asserts an effect on the MAP, and neither would notice if `IsRadarJammed` always returned false. Three bite tests, each on its own stage; the save stage fails while `saveload` passes throughout - the same blind spot ADR-063 found, which is why each power carries its own save stage. **The save-surgery helper lagged for the SIXTH time**, caught in the battery exactly where its own comment predicts every time |
| P7-25 | GDD s3 line 30's TUNNEL DEPLOYMENT, the Sodality's second dirty trick - and the first support power that MOVES ENTITIES | GDD s3 line 30; ADR-060; ADR-064; Q021 | **Measuring decided the design, fourth wave running: it needed NOTHING NEW.** CarrierCapacity bounds it, SpawnOffsets places it, SightCells is its reach, IsVisible is its counterplay - all four already existed, so no new state, no save bump, no invented number. Carried by the VEIL PROJECTOR: the building that hides things is where concealed transit belongs, and it answers ADR-060's measured finding that nobody ever builds one, WITHOUT changing a single number. Rejected: the Watch Post, because powers share a charge (ADR-064) and putting two of the Sodality's three tricks in one killable building is too much counterplay in one place; the generator (infrastructure, and the first building raised); the seismic charge (technically excluded, uses ChargeTicks) | **ONE moves, measured: `veil 2026` -> 0xA5D040C135D43B67**, and it is NOT a behaviour change - `Add` seeds a power's charge at spawn and ChargeTicks is hashed, so a Veil that now carries a power hashes differently the instant it exists. The scenario's own assertion passes verbatim. Save UNCHANGED at v14 (the power holds no live state); **catalogue checksum 0xD69565C84EE1166E** | **DONE** - ADR-066; `tunneldeploymentgate` (5 stages, CONTROL FIRST). First power that moves entities, so no other gate would notice units landing on each other or marching back across the map. Three bite tests, each on its own stage. The SIGHT RULE is checked FIRST so a blind aim spends the charge and moves nobody - otherwise the power quietly doubles as a free scout. **ONE POWER LEFT: the decoy army**, and it needs per-viewer entity visibility the sim does not have - refusing it with the argument recorded is a legitimate outcome |
| P7-26 | GDD s3 line 30's DECOY ARMY, the LAST of the five named powers - and the one this project had twice written down as probably unbuildable | GDD s3 line 30; Q021; ADR-062 to ADR-066 | **THE ASSUMPTION WAS WRONG.** A decoy does NOT need per-viewer entity visibility: it needs to BE a real entity that looks like a real unit and does nothing, so an observer cannot tell which army is which UNTIL THEY SHOOT IT. The deception lives in the observer's uncertainty, not in the renderer - which is also how the benchmark games did it, their fakes being real objects with no function. `SpawnUnit` already takes hp, weaponId, sightCells and unitType separately, so there was no architecture to grow. Count = **one WAVE's worth** (the game's own number for an army worth attacking); hp = **1**, the smallest thing that can exist; **no weapon, no sight** - a hollow imitation has no eyes, and a power that doubled as a scout would be two powers. Carried by the SHROUD NEST (a shroud is deception; cheap and early, suiting "cheap infantry swarms") | **All 24 byte-identical, measured**; save UNCHANGED at v14 (decoys are ordinary entities); **catalogue checksum 0xFA2DFE52192660BA** | **DONE** - ADR-067; `decoyarmygate` (4 stages, CONTROL FIRST). **The gate's first TWO versions were wrong the same way and the bite tests found it**: it identified decoys by hit points, then by "no weapon and no eyes" - both properties it asserts - so breaking the rule a later stage tested made decoys UNFINDABLE and the count stage failed first with the wrong message. A gate whose stages can only fail in one order is one assertion wearing four hats. Fixed by identifying decoys BY PROVENANCE, independent of everything asserted. **Q021 IS CLOSED: all five named support powers exist**, and five shipped with no balance argument because every number belongs to something already in the game |
| P7-27 | **COMPLETION AUDIT.** The support-power arc closed, so this wave re-read the tracker and doc 24 to choose the next row - and found the roadmap itself had gone stale | doc 24 tiers A-D; GDD s8 line 71 | No design call taken. The audit MEASURED doc 24's claims against the catalogue rather than reading them | **All 24 byte-identical** - nothing in /sim changed; a reporting mode and document edits only | **DONE** - ADR-068; `parityprobe` (derived, NON-ASSERTING by ADR-061's gate-vs-probe rule: counts are balance, not correctness). **DOC 24's TIER B WAS WRONG IN THREE PLACES:** B1 said "the turret is the only defence in the game" (there are TWELVE defence buildings), B4 said "No hero unit" (both Commandos ship), B6 said mines were "NOT TAKEN, blocked on the Producer" (com_mine ships and minegate asserts its shape - ADR-044 later cited that gate). The phase's signature defect, in the phase's own analysis of record, and **the project already knew** - the tracker header warns the design docs "lag by whole waves", which is a defect written down as a warning instead of turned into a check. Also corrected THIS tracker: P7-10 still read "wall tiers untouched and still open" after ADR-061 refused them. **P7 IS ONE GAME DESIGNER SENTENCE AND ONE PLAYTEST FROM COMPLETE**: the only parity gap left is the Directorate's 2 support powers against s8's "3-4", and the sixth power has no name anywhere - filed as **Q022**, because a number can be derived and a name cannot |
| P7-28 | **THE PLAYTEST KIT.** Seventeen ADRs name a playtest as the thing that would settle them and NOT ONE made it easier to do one - a tooling gap, not a design gap | doc 24; the 17 ADRs' own overturning clauses | **The brief for a script.** Checking existing artefacts first found `P6-playtest-brief-2026-07-24.md` - the format already existed and had been used: "Play this" plus "what to try, and the question each answers". A shell script that drives the client is the WRONG instrument; a playtest is a human with the game, and what makes it easy is a page. So: a P7 brief in the proven format, DERIVED from the ADRs' own overturning clauses, plus a derived index so it cannot go stale | **All 24 byte-identical** - docs and one tools script; nothing in /sim or /game | **DONE** - `docs/tickets/P7-playtest-brief-2026-08-03.md` (4 matches, ~90 minutes, ordered so stopping early still answers something) and `tools/playtest-index.sh`, which derives from docs/ which decisions await a human verdict and flags any the brief omits. **THE INDEX CAUGHT THREE OMISSIONS IN THE BRIEF ON ITS FIRST REAL RUN**, including ADR-061's own overturning condition which I had written eight waves earlier - recorded in the brief rather than quietly fixed, because the point is that a brief written from memory WILL have holes. Its first output also buried the signal (21 "not cited", mostly prose), so it now splits pending DECISIONS from passing mentions. Non-asserting and deliberately NOT in the CI battery, per ADR-061's gate-vs-probe rule |
...
| P7-10 | Wall GATES (the "wall tiers" half **was closed as REFUSED by ADR-061, P7-19**; this row read "untouched and still open" until P7-27 audited it) | B7 | ~~C6b~~ - resolved WITHOUT an override: ADR-005 clause 6 stands, and the P7-10 amendment records that its blocker is scoped to SIMULTANEOUS per-player passability, which a single global open state does not need | goldens NEUTRAL, measured (predicted to move, and did not); catalogue checksum MOVES; save format v12 | **PART DONE** - ADR-005 amendment; wallgategate (8 stages incl. the enemy walking through an open gate, asserted as the DESIGN); client harness 194 checks |
