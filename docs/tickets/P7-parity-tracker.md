...
| P7-7c | **NEXT: the commander converts income into army.** "Infrastructure before army, always" does not terminate when the ladder grows, so two economy rows have made it progressively less able to fight | B2 | - | expect goldens to move | pending - **this is the row that unblocks P7-7b**, and the evidence for it is in ADR-048 |
