...
- P6 is behind this and its tracker (docs/tickets/P6-campaign-tracker.md) is history rather than a resume point. Of the items it listed as open, the air layer shipped under ADR-028 (bespoke art still owed), and the rest still need a human: C8 multi-resource (Q014) and C9 faction recipe (Q017). **C6b wall gates shipped** (P7-10): ADR-005 clause 6's blocker turned out to be scoped to SIMULTANEOUS per-player passability, which a gate with one global open/closed state does not need - so it was sidestepped rather than overridden, and clause 6 stands untouched with an amendment recording the distinction and its price (an enemy can follow you through an open gate).
- **What P7 needs from a human, and it is the same three things every wave keeps arriving at:** a PLAYTEST (four-player free-for-all, six campaign missions and a LAN match on a four-seat map all work and nobody has played any of them); one design sentence each for P7-8c (what an alliance IS, which 4v4 cannot be built without) and P7-11b/c (the hero's ability, and whether mines are wanted at all); and a decision on whether golden hashes may be REGENERATED, which is a replay-compatibility break and which the remaining engineering rows require.
